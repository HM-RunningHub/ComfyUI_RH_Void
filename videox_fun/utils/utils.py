...
def save_videos_grid(videos: torch.Tensor, path: str, rescale=False, n_rows=6, fps=12, imageio_backend=True, color_transfer_post_process=False):
    ...
    if imageio_backend:
        if path.endswith("mp4"):
            ...
            writer = imageio.get_writer(path, duration=(1000 * 1 / fps))
        ...
        try:
            for x in videos:
                ...
                if rescale:
                    x = (x + 1.0) / 2.0  # -1,1 -> 0,1
                frame = (x * 255).cpu().float().numpy().astype(np.uint8)

                if color_transfer_post_process:
                    if reference_frame is None:
                        reference_frame = frame.copy()
                    else:
                        frame = color_transfer(frame, reference_frame)
        ...
        if color_transfer_post_process and outputs:
            reference_image = np.uint8(outputs[0])
            for i in range(1, len(outputs)):
                outputs[i] = Image.fromarray(color_transfer(np.uint8(outputs[i]), reference_image))
...
def _resolve_prompt_json_path(data_rootdir, input_video_name):
    """Resolve prompt.json for a sequence, with fallback to a shared prompt under data_rootdir."""
    seq_prompt_path = os.path.join(data_rootdir, input_video_name, "prompt.json")
    if os.path.isfile(seq_prompt_path):
        return seq_prompt_path

    root_prompt_path = os.path.join(data_rootdir, "prompt.json")
    if os.path.isfile(root_prompt_path):
        return root_prompt_path

    fallback_paths = sorted(glob.glob(os.path.join(data_rootdir, "**", "prompt.json"), recursive=True))
    if fallback_paths:
        logger.warning(
            f'prompt.json not found for sequence "{input_video_name}", '
            f'falling back to shared prompt: {fallback_paths[0]}'
        )
        return fallback_paths[0]

    raise FileNotFoundError(
        f'No prompt.json found for sequence "{input_video_name}" under {data_rootdir}'
    )
...
def get_video_mask_input(
    ...
    prompt_path = _resolve_prompt_json_path(data_rootdir, input_video_name)
    with open(prompt_path, "r", encoding="utf-8") as prompt_file:
        prompt = json.load(prompt_file)['bg']
...
def get_video_mask_input_from_paths(
        video_path,
        quadmask_path,
        prompt,
        sample_size,
    ...
    input_video = media.read_video(video_path)
    clip_image = Image.fromarray(np.array(input_video[0]))
    ...
    input_mask = torch.from_numpy(media.read_video(quadmask_path)).float()[:max_video_length]
    ...
    if use_quadmask:
        ...
        input_mask = torch.where(input_mask > 191, 255, input_mask)
        input_mask = 255 - input_mask
        logger.debug('[QUADMASK INFERENCE] Using 4-value quadmask: [0, 63, 127, 255]')
    else:
        input_mask = torch.where(input_mask > 192, 255, input_mask)
        input_mask = torch.where((input_mask <= 192) * (input_mask >= 64), 128, input_mask)
        input_mask = torch.where(input_mask < 64, 0, input_mask)
        input_mask = 255 - input_mask
        logger.debug('[TRIMASK INFERENCE] Using 3-value trimask: [0, 127, 255]')
